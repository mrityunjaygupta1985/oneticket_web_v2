    ...
    return (
        <div className={`fixed bottom-6 right-6 z-[60] flex flex-col items-end gap-4`}>
            ...
            <div className="flex items-center gap-4">
                ...
                {/* Floating Image Icon Button - INCREASED SIZE */}
                <button 
                    onClick={() => {
                        setIsOpen(!isOpen);
                        if (!isOpen) setShowPrompt(false);
                    }}
                    className="w-28 h-28 hover:scale-110 transition-transform will-change-transform relative group focus:outline-none"
                >
                    <img 
...
const App: React.FC = () => {
  ...
  // Header Scroll Animation (Direct DOM manipulation for performance)
  useEffect(() => {
    if (!showNotification) {
    ...
    const updateHeader = () => {
      if (headerRef.current) {
        // Adjust header top position based on scroll to mimic sticky behavior relative to notification bar
        const top = Math.max(0, 50 - window.scrollY);
  ...
  // Hero Section Scroll Animation (GSAP for smoothness)
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
        ...
        if (heroRef.current) {
            ...
            // Simultaneously animate scale and opacity
            tl.to(heroRef.current, { scale: 0.9, duration: 0.4, ease: "none" }, 0)
              .to(heroRef.current, { opacity: 0, duration: 1, ease: "none" }, 0);
        }
    });
    return () => ctx.revert();
  }, []);
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${isDarkMode ? 'dark bg-slate-900 text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      {showNotification && (
        <div className="bg-slate-900 text-white px-4 py-3 text-center text-sm font-medium relative z-50 flex justify-center items-center gap-4 border-b border-white/5 h-[50px]">
           <div className="flex-1 flex justify-center gap-2">
             <span>Auto and Cabs are Live Now in Bangaluru!!</span>
      ...
      <main>
        ...
        <section 
            ref={heroRef}
            className="sticky top-0 h-screen flex flex-col items-center justify-center relative pb-32 overflow-hidden z-0" 
            // Removed inline transform/opacity driven by state scrollY
        >
